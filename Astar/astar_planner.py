...
class AStarPlanner(object):
    ...
    def plan(self, start_state, dest_state):
        ...
        # creates array based off of world.shape values, then initializes optimal to 0
        # np.ones creates a new array of given size and shape, filled with ones
        # at start, only start has value of 0, everything else is infinite distance
        best_dist_found = float("inf") * np.ones((world.shape[1], world.shape[0]))
        best_dist_found[start_state.x, start_state.y] = 0

        fv = float("inf") * np.ones((world.shape[1], world.shape[0]))
        fv[start_state.x, start_state.y] = sqrt((start_state.x - dest_state.x) ** 2 + (start_state.y - dest_state.y) ** 2)
        ...
        while Q:
            ...
            if s == dest_state:
                # follow returns shortest path by following parent pointers
                # aka backtrack from End to Start, this returns the plan.
            ...
            for ns in self.get_neighboring_states(s):
                ...
                transition_distance = best_dist_found[s.x, s.y] + sqrt((ns.x - start_state.x)**2 + (ns.y - start_state.y)**2)

                heuristic = sqrt((ns.x - dest_state.x)**2 + (ns.y - dest_state.y)**2)
                total_cost = transition_distance + heuristic

                alternative_best_dist_ns = best_dist_found[s.x, s.y] + total_cost
                ...
                if ns not in Q or best_dist_found[s.x, s.y] + sqrt((ns.x - s.x)**2 + (ns.y - s.y)**2) < best_dist_found[ns.x, ns.y]:
                    best_dist_found[ns.x, ns.y] = best_dist_found[s.x, s.y] + sqrt((ns.x - s.x)**2 + (ns.y - s.y)**2)
